 ...
 */
export async function openIncreasePosition({
  agent,
  indexToken,
  amountIn,
  leverage,
...
} & OpenIncreasePositionParams): Promise<string> {
  ...
  const sizeDeltaValue = sizeDelta;
  ...
  await walletClient.sendTransaction({
    ...
    args: [BMX_ROUTER_ADDRESS, amountInBigInt],
  });
 ...
 */
export async function closeDecreasePosition({
  agent,
  indexToken,
  percentage,
  isLong,
  ...
  agent: Agentix<EvmWalletBase>;
} & CloseDecreasePositionParams): Promise<string> {
  const walletClient = agent.wallet;
...
/**
 * View current position details for a specific token
 */
export async function getPosition({
  ...
  agent: Agentix<EvmWalletBase>;
} & GetPositionParams): Promise<string> {
  const walletClient = agent.wallet;
  ...
  const formattedPosition = {
    size: `${Number(size) / 10**30} USD`,
    collateral: `${Number(collateral) / 10**30} USD`,
    averagePrice: `${Number(averagePrice) / 10**30} USD`,
    realisedPnl: `${Number(realisedPnl) / 10**30} USD`,
    lastIncreasedTime:
