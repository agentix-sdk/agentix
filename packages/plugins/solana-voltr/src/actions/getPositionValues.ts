...
import { z } from "zod";
import { voltrGetPositionValues } from "../tools/voltr_get_position_values";

const getVoltrPositionValuesAction: Action<SolanaWalletBase> = {
  ...
  handler: async (agent: Agentix<SolanaWalletBase>, input: Record<string, any>) => {
    try {
      ...
      const result = await voltrGetPositionValues(agent, vault);
