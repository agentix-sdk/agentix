...
async function getOrCreateVaultDepositor(agent: Agentix<SolanaWalletBase>, vault: string) {
...
async function getVaultAvailableBalance(agent: Agentix<SolanaWalletBase>, vault: string) {
  try {
...
*/
async function getIsOwned(agent: Agentix<SolanaWalletBase>, vault: string) {
  try {
    const { vaultClient, cleanUp } = await initClients(agent);
    const vaultPublicKey = new PublicKey(vault);
    const vaultDetails = await vaultClient.getVault(vaultPublicKey);
    const isOwned = vaultDetails.manager.equals(
      new PublicKey(agent.wallet.getAddress()),
    );
 ...
 * @param name
 */
export async function getVaultAddress(agent: Agentix<SolanaWalletBase>, name: string) {
