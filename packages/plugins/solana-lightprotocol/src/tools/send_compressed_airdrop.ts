...
export async function sendCompressedAirdrop(
  ...
  const url = agent.wallet.getConnection().rpcEndpoint;
  ...
  try {
    await getAssociatedTokenAddress(mintAddress, publicKey);
  } catch (_error) {
...
async function processAll(
  ...
  const lookupTableAccount = (
    await agent.wallet.getConnection().getAddressLookupTable(lookupTableAddress)
  ).value!;
