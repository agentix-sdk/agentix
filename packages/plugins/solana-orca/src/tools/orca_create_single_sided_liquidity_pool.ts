 ...
 *
 * @param agent - The `Agentix` instance representing the wallet and connection details.
...
export async function orcaCreateSingleSidedLiquidityPool(
  ...
  try {
    ...
    if (agent.wallet.getConnection().rpcEndpoint.includes("mainnet")) {
      whirlpoolsConfigAddress = new PublicKey(
        "2LecshUwdy9xi7meFgHtFJQNSKk4KdTrcpvaB56dP2NQ",
      );
    } else if (agent.wallet.getConnection().rpcEndpoint.includes("devnet")) {
      whirlpoolsConfigAddress = new PublicKey(
    ...
    const baseParamsPool = {
      ...
      funder: new PublicKey(agent.wallet.getAddress()),
    };
    ...
    const baseParamsLiquidity = {
      ...
      tokenMaxB,
      whirlpool: whirlpoolPda.publicKey,
      positionAuthority: new PublicKey(agent.wallet.getAddress()),
