...
export async function manifestCreateMarket(
  ...
  const createAccountIx: TransactionInstruction = SystemProgram.createAccount({
    ...
    lamports: await agent.wallet.getConnection().getMinimumBalanceForRentExemption(
 ...
 * @returns Transaction signature
 */
export async function withdrawAll(agent: Agentix<SolanaWalletBase>, marketId: PublicKey) {
  try {
    const mfxClient = await ManifestClient.getClientForMarketNoPrivateKey(
      agent.wallet.getConnection(),
      marketId,
